...
from instruments.ni6738 import NI6738
from instruments.senis3MTS.field_sensor_sync import FieldSensor
from instruments.vna import VNA_ZNA
...
class MeasurementController:
    ...
    def __init__(self):
        ...
        self.ps: Optional[PowerSupply] = None
        self.sensor: Optional[FieldSensor] = None
    ...
    def initialize_setup(self, setup_params: SetupParameters):
        ...
        # 2. Power Supply configuration setup
        if self.setup_params.ps_port:
            logger.info(f"[Controller] Connecting to Power Supply on {self.setup_params.ps_port}...")
            self.ps = PowerSupply(self.setup_params.ps_port, self.setup_params.ps_baud)
            self.ps.setOffsetConversion(offset=self.setup_params.ps_offset, conversion=self.setup_params.ps_conversion)
        else: 
        ...
        self.sensor = FieldSensor()
        if (not self.sensor.is_connected()):
            logger.warning("[Controller] Connection to magnetic flied sensor failed")
